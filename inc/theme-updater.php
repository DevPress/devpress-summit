...
class Summit_Theme_Updater{
	...
	public function theme_source_selection( $source, $remote_source, $upgrader ){
		...
		if( isset( $upgrader->skin->theme_info->template ) ){
			...
			if ( $upgrader->skin->theme_info->template == get_template() ){
				...
				$upgrader->skin->feedback( __( 'Executing upgrader_source_selection filter function...', 'summit' ) );
				...
				if( isset( $source, $remote_source, $theme_name ) ){
					...
					if(@rename( $source, $new_source ) ){
						$upgrader->skin->feedback( __( 'Renamed theme folder successfully.', 'summit' ) );
						return $new_source;
					}
					/* unable to rename the folder to correct theme folder */
					else{
						$upgrader->skin->feedback( __( '**Unable to rename downloaded theme.', 'summit' ) );
						return new WP_Error();
					}
				}
				/* fallback */
				else
					$upgrader->skin->feedback( __( '**Source or Remote Source is unavailable.', 'summit' ) );
			}
		}
		return $source;
	}
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . __( ' Theme Updates', 'summit' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. __( 'Username: ', 'summit' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Email: ', 'summit' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Key: ', 'summit' ) . '<code>' . $key . '</code></p>';
			}

			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				_e( '<p>Your theme update is <span style="color:green">active</span></p>', 'summit' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'summit' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Edit Key', 'summit' ) . '</a></p>';
			}
			/* else */
			else{
				_e( '<p>Unable to validate update activation.</p>', 'auto-hosted' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Try again', 'summit' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Add Key', 'summit' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php _e( 'User name', 'summit' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Email', 'summit' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Activation Key', 'summit' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
